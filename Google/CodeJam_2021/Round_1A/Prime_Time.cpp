...
void check(int rest_sum, int mult, vi store, int &matched) {
    // pl("Entered check"); deb(rest_sum); deb(mult);
    if(rest_sum == mult) {
        if(rest_sum > matched) {
            matched = rest_sum;
        }
    ...
    if (mult > rest_sum) {
        ...
        For(i, 0, store.size()) {
            ...
            check(rest_sum - temp, mult*temp, store, matched);
...
void sol(int ind) {
    ...
    vi poss;
    int matched = 0;
    check(sum, mult, store, matched);

    cout << "Case #"<<ind<<": "<<matched<<endl;
    return;
}
