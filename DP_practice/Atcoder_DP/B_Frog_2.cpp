...
int32_t main() {
    ...
    vi arr = input1l();
    vi2d dp(k+1, vi(n+1, INT_MAX));
    dp[0][1] = 0;
    For(i, 1, k+1) {
        dp[i][1] = abs(arr[i]-arr[0]);
    }
    // dp[1][1] = abs(arr[1] - arr[0]);
    // dp[2][1] = abs(arr[2]-arr[0]);
    int temp = 0;
    For(i, 2, n+1) {
        dp[0][i] = dp[1][i-1];
        For(j, 1, k+1) {
            ...
            if(i <= n-j) {
                if(j == k) {
                    dp[j][i] = dp[0][i] + abs(arr[i-1+j] - arr[i-1]);
                }
                else {
                    temp = dp[0][i] + abs(arr[i-1+j] - arr[i-1]);
                    dp[j][i] = min(temp, dp[j+1][i-1]);
                }
            }

        }
        
        // if(i<=n-1) dp[1][i] = dp[0][i] + abs(arr[i]-arr[i-1]);
        // if(i<= n-2) dp[2][i] = dp[0][i] + abs(arr[i+1] - arr[i-1]);
    }
    // pv2d(dp);
    cout << (dp[0][n]) << endl;
}