...
class Solution {
public:
    void calc ( vi store, vi2d &grid, int side, )
    vector<int> getBiggestThree(vector<vector<int>>& grid) {
        vector<int> store(3, -(1<<30));
        int local_min = -(1<<30);
        int m = grid.size();
        int n = grid[0].size();
        int limit = (m > n) ? n : m;
        int side = (m > n) ? 0 : 1;
        int hand;
        for(int need = 1; need <= limit; need += 2) {
            hand = (need + 1) / 2;
            for(int i = hand-1; i <= n-hand; i++) {
...
int main() {
    ...
    int start = 5;
    cout << "Answer : " <<endl ;
    pv( mySolution.getBiggestThree(grid) );
}