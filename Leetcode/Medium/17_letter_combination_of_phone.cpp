...
class Solution {
public:
    unordered_map<int, vector<char>> mapp;
    string given="";
    vector<string> ans;

    void recur(int len, string str) {
        // deb(given.size());deb(len); deb(str);
        if(len == given.size()) {
            ans.pb(str);
            return;
        }
        else {
            For(i, 0, mapp[given[len]-'0'].size()) {
                recur( len+1, str + mapp[given[len]-'0'][i]);
            }
        }
        return;
    }
    ...
    vector<string> letterCombinations(string digits) {
        ...
        mapp.insert({5, {'j', 'k', 'l'}});
        mapp.insert({6, {'m', 'n', 'o'}});
        mapp.insert({7, {'p', 'q', 'r', 's'}});
        mapp.insert({8, {'t', 'u', 'v'}});
        mapp.insert({9, {'w', 'x', 'y', 'z'}});   
        string str = "";
        recur(0, str);
        return ans;
    }
...
int main() {
    ...
    string str = "abcdefg";
    cout << "Answer : " << endl;
    pv(mySolution.letterCombinations("23") );
}