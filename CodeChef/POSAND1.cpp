...
int basePowerOfTwo(int n) {
    return (int)floor(log(n)/log(2));
}

void fillup(deque<int> &arr, int n) {
    int base = 4;
    while(base <= pow(2,basePowerOfTwo(n) ) ) {
        int i = base+2;
        arr.pb(base+1);arr.pb(base);
        while(i < base*2 && i <= n) {
            arr.pb(i);
            i++;
        }
...
void sol() {
    inpt(n);
    if(n == 1) pl("1");
    else if( basePowerOfTwo(n) == ((int)ceil(log(n)/log(2)) ) ) pl("-1");
    else if(n == 3) pl("1 3 2");
    else {
        deque<int> arr = {2,3,1};
        fillup(arr, n);
        loop(it, arr) {
