...
void sol() {
    ...
    For(i, 0, n) {
        For(j, 0, q) {
            total += (tree[i].first-well[j].first)*(tree[i].first-well[j].first);
            total %= MOD;
            total += (tree[i].second-well[j].second)*(tree[i].second-well[j].second);
            total %= MOD;
        }
    }
    pl(total); return;
...
int32_t main() {
    // SPEED1;
        ...
        freopen("watering_well_chapter_1_1_output.txt", "w", stdout);
    #endif
    inpt(t);
    For(i, 1, t+1) {
        cout<< "Case #" << i<<": ";
