...
bool check(vector<vector<char>> &arr, int x, int y) {
    ...
    for(i=x1 , j=y1; i>=0 && j<arr.size(); i--, j++) {
        // pl("33"); 
        if(arr[i][j] == 'q') return 0;        
    }
    // cout << "Returning 1\n";
    vector<int> l = {-2, -1, 1, 2}, r = {-1, -2, -2, -1};
    For(k, 0, 4) {
        // pl("44"); 
        i = x1 + r[k]; j = y1 + l[k];
        // cout << " i & j : " << i << " , " << j << endl;
        if(i>=0 && i<arr.size() && j >= 0 && j < arr.size() && arr[i][j] == 'q') {
            return 0; 
        }
    }
    return 1;
...
void nqueen(vector<vector<char>> &arr, int level ) {
