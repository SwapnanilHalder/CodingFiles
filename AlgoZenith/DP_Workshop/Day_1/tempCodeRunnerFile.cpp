...
void whole(vector<char> &arr, int pivot, int end) {
    // cout << "Entered : " << pivot << " ; " << end << endl;
    // pv(arr);
    for(; pivot < end-1; pivot++) {
        pv(arr);
        swap(arr[pivot], arr[pivot+1]);
    }
}

void recur(vector<char> &arr, int pivot, int end) {
    for(; end > 0 && pivot >= 0; end -= 2, pivot--)
        whole(arr, pivot, end);
    pv(arr);
}
