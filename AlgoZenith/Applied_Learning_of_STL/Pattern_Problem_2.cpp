...
void print_2d(vector<vector<char>> &a) {
    for(auto &i: a) {
        for(auto &j: i) {
            cout << j;
        }
...
void solve() {
    int row, col, unit; cin >> row >> col >> unit;
    vector<vector<char>> arr(unit+1, vector<char> ((unit+1)*col +1, '*'));
    vector<vector<char>> lr_unit(unit, vector<char>(unit+1, '.'));
    vector<vector<char>> rl_unit(unit, vector<char>(unit+1, '.'));
