...
void sol() {
    ...
    deb(mapped.size());
    int deleted {-1};
    int done = 0; int h = 0; int row=0;
    while (done < n) {
        auto it = mapped.end(); it--;
        while(it->first != 0) {
            pmint(mapped);
            cout << "    NOW : " << it->first << " has come\n";
            if ((it->first + row) > w ) {
                break;
            }
            row += it->first;
            ...
            if(it->second == 0) {
                deleted = it->first;
                mapped.erase(it->first);
                // it = mapped.lower_bound((int)deleted/2);
                it--;
            }
            
            done++; deb(done);
            cout << "POINTED AT : " << it->first << endl;
            if(done == n) {
                cout << "DONE == n\n";
                break;
            }
