...
void sol() {
    ...
    int mid = -1;
    if (mid == -1) {
        int incr = 0, eql = 0, decr = 0;
        For(i, 0, n-1) {
            if (arr[i] < arr[i+1]) incr++;
            if (arr[i] > arr[i+1]) decr++;
            if (arr[i] == arr[i+1]) eql++;
        }
        if(eql != 0) {
            if(incr != 0) {
                pl("-1"); return;
    ...
    if(up_mid != -1) {
        int i = up_mid;
        int low_diff = (arr[i] - arr[i-1]); 
        int supposed_m = (arr[i] + low_diff) - arr[i+1];
        // deb(supposed_m);
        if(arr[0] > low_diff) {
            pl("-1"); return;
        }
        For(i, 0, n-1) {
            if( (arr[i] + low_diff) % supposed_m != arr[i+1] ) {
                pl("-1"); return;
            }
        }
        cout << supposed_m  << " " << low_diff << endl; 
    }
    else {
        For(i, 1, n-1) {
            if ( (arr[i] < arr[i+1]) && (arr[i] < arr[i-1]) ) {
        ...
        if(down_mid != -1) {
            int i = down_mid;
            int low_diff = (arr[i+1] - arr[i]); 
            int supposed_m = (arr[i-1] + low_diff) - arr[i];
            // deb(supposed_m);
            if(arr[0] > low_diff) {
                pl("-1"); return;
            }
