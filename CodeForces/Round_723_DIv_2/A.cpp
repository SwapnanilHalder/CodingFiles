...
void correct(vi &arr, int n) {
    if((double)arr[0] == ((double)arr[n-1] + arr[1]) / 2) {
        swap(arr[0], arr[1]);
    }
    if((double)arr[n-1] == ((double)arr[n-2] + arr[0]) / 2) {
        swap(arr[0], arr[n-1]);
    }
    For(i, 1, n-1) {
        if((double)arr[i] == ((double)arr[i-1] + arr[i+1]) / 2) {
            swap(arr[i], arr[i+1]);
        }
    }
}
